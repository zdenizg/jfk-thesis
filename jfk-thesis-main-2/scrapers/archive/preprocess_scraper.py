...
class PDFScraper:
    ...
    def clean_text(self, text):
            ...
            """
            # 1. Remove document headers/footers and OCR artifacts BEFORE lowercasing
            text = re.sub(r'104-\d+-\d+', '', text)  # Remove document IDs like "104-10065-10050"
            text = re.sub(r'---\s*page\s*\d+\s*---', '', text, flags=re.IGNORECASE)  # Remove page markers
            text = re.sub(r'20\d{2}\s+release\s+under.*?act\s+of\s+\d{4}', '', text, flags=re.IGNORECASE | re.DOTALL)
            
            # 2. Remove classification markings (SECRET, CONFIDENTIAL, etc.)
            text = re.sub(r'\b(secret|confidential|classified|top\s+secret)\b', '', text, flags=re.IGNORECASE)
            
            # 3. Remove common OCR artifacts and noise
            text = re.sub(r'\b[a-z]{1,2}\b(?:\s+[a-z]{1,2}\b)+', ' ', text)  # Remove scattered single/double letters like "a e r s"
            text = re.sub(r'[^\w\s\.\,\;\:\?\!]', ' ', text)  # Keep more punctuation for sentence structure
            
            # 4. Fix common OCR errors
            text = re.sub(r'\s+([.,;:!?])', r'\1', text)  # Fix spacing before punctuation
            text = re.sub(r'([.,;:!?])([a-zA-Z])', r'\1 \2', text)  # Add space after punctuation
            
            # 5. Lowercase text (do this AFTER classification removal to catch all variants)
            text = text.lower()
            
            # 6. Remove extra whitespace and normalize
    ...
    def extract_text_ocr(self, pdf_path, dpi=500, lang='eng'):
        """
    ...
    def scrape_pdf(self, pdf_path, output_path=None, method='auto', clean=False, **kwargs):
        """
        ...
        Args:
            ...
            clean: Whether to apply text cleaning (default: False)
    ...
    def scrape_to_csv(self, directory_path, csv_output_path, clean=False, **kwargs):
            """
            Scrape all PDFs in directory and save to CSV file
            ...
            for i, pdf_file in enumerate(pdf_files, 1):
                ...
                try:
                    # Scrape PDF
                    text = self.scrape_pdf(str(pdf_file), output_path=None, clean=clean, **kwargs)
...
if __name__ == "__main__":
    ...
    output_csv = "/Users/furkandemir/Desktop/Thesis/files/1/simple/extracted_texts.csv"
    
    # Run the scraper with text cleaning enabled
    df = scraper.scrape_to_csv(
        ...
        clean=True,  # Enable text cleaning
